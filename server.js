...
import serviceAccountKey from "./blog-website-82c10-firebase-adminsdk-xbh4l-37681d5fd0.json" assert { type: "json" };
...
const genrateUserName = async (email) => {
  ...
  let usernameExsist = await User.exists({
    "personal_info.username": username,
  }).then((result) => result);

  usernameExsist ? (username += nanoid().substring(0, 5)) : "";
...
server.post("/signup", (req, res) => {
  ...
  if (fullname.length < 3) {
    return res
      .status(403)
      .json({ error: "Fullname must be atleast 3 letter long" });
  }
  if (!email.length) {
  ...
  if (!passwordRegex.test(password)) {
    return res.status(403).json({
      error:
        "Password should be 6 to 20 chracters long with a numeric, 1 lowercase and 1 uppercase letter",
    });
  }
  ...
  bcrypt.hash(password, 10, async (err, hashed_password) => {
    ...
    user
      ...
      .catch((err) => {
        if (err.code === 11000) {
          return res.status(500).json({ error: "Email already exsist" });
        }
  ...
  User.findOne({ "personal_info.email": email })
    .then((user) => {
      ...
      if (!user.google_auth) {
        ...
        return res.status(403).json({
          error: "Account Created using google. Try logging with google.",
        });
      }
    })
    .catch((err) => {
...
server.post("/google-auth", async (req, res) => {
  ...
  getAuth()
    ...
    .then(async (decodedUser) => {
      ...
      let user = await User.findOne({ "presonal_info.email": email })
        .select(
...
server.post("/change-password", verifyJWT, (req, res) => {
  ...
  if (
    ...
    return res.status(403).json({
      error:
        "Password should be 6 to 20 chracter long, should contain atleast one uppercase letter, one lowercase letter and one number",
    });
  }
  ...
  User.findOne({ _id: req.user })
    .then((user) => {
      if (user.google_auth) {
        return res
          .status(403)
          .json({ error: "You can not change password of a google account" });
      }
...
server.post("/update-profile", verifyJWT, (req, res) => {
  ...
  User.findOneAndUpdate({ _id: req.user }, updateObj, {
    runValidators: true, //to ensure username is still unique
  })
    .then(() => {
    ...
    .catch((err) => {
      if (err.code === 11000) {
        return res.status(500).json({ error: "Username already exsist" });
      }
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (!draft) {
    if (!des.length || des.length > 200) {
      return res.status(403).json({
        error: "You must provide a blog description under 200 chracters",
      });
    }
    ...
    if (!tags.length || tags.length > 10) {
      return res.status(403).json({
        error: "Provide tags in order to publish the blog, Maxium 10",
      });
    }
  }
...
server.post("/get-blog", (req, res) => {
  ...
  let incrementalVal = mode !== "edit" ? 0 : 1;
  ...
  Blog.findOneAndUpdate(
    ...
    .then((blog) => {
      ...
      if (blog.draft && !draft) {
        return res
          .status(403)
          .json({ error: "You can not acess draft blogs " });
      }
